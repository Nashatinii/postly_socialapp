...
export default function ProfilePage() {
    ...
    const handleProfileImageChange = async (event) => {
        ...
        if (file) {
            ...
            try {
                console.log(file);
                const response = await axios.put("http://localhost:5000/api/users/user/Pic",{profilePic : file.name}, {
                    withCredentials: true,
                });
    ...
    const handleEmailChange = async () => {
        if (newEmail) {
            try {
                ...
                const response = await axios.put("http://localhost:5000/api/users/user/email",email , {
                    withCredentials: true,
                });
    ...
    const handlePasswordChange = async () => {
        if (newPassword) {
            try {
                ...
                const response = await axios.put("http://localhost:5000/api/users/user/password",password , {
                    withCredentials: true,
                });
...
/>
             <div className="user-name">
                    ...
                    <div className="profile-button">
                        <button className="editInfo" onClick={() => setShowEditEmail((prev) => !prev)}>
                            Edit Email
                        </button>
                        {showEditEmail && (
                            <div>
                        ...
                        <button className="editInfo" onClick={() => setShowEditPassword((prev) => !prev)}>
                            Edit Password
                        </button>
                        {showEditPassword && (
                            <div>
        ...
        {
        currentUser && currentUser.username && userPosts.map((post) => (
